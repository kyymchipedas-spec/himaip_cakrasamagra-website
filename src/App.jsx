...
function IconInstagram() {
  return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="5"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none"/></svg>;
}
function IconTiktok() {
  return <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.69a8.18 8.18 0 0 0 4.78 1.52V6.75a4.85 4.85 0 0 1-1.01-.06z"/></svg>;
}
function IconEmail() {
  return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m2 7 10 7 10-7"/></svg>;
}
      ...
      <div style={styles.footerInner}>
        ...
        <div style={styles.footerSosmed}>
          ...
          <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
            ...
            <a href={SOSMED.tiktok} target="_blank" rel="noreferrer" style={styles.hima_ip_stisiptasik} title="TikTok">
              <IconTiktok />
              <span>TikTok</span>
            </a>
...
export default function App() {
  ...
  async function addMember(name, npm, position, photoFile, cropData) {
    setBusy(true);
    let photoUrl = null;
    if (photoFile) {
      try { photoUrl = await compressImage(photoFile, 800, 0.85); } catch (e) { console.error(e); }
    }
    const member = { id: "m_" + Date.now() + Math.random().toString(36).slice(2, 7), name: name.trim(), npm: npm.trim(), position: position.trim(), photoUrl, cropData: cropData || null };
    ...
    <div style={styles.page}>
      ...
      <header style={styles.header}>
        <div style={styles.headerInner}>
          <div style={styles.brandRow} onClick={() => { setTab("beranda"); setActiveEvent(null); }} title="Kembali ke Beranda" role="button" tabIndex={0} onKeyDown={(e) => e.key === "Enter" && setTab("beranda")} >
          ...
          <nav style={styles.nav}>
            {[["galeri","Galeri Kegiatan"],["anggota","Anggota"]].map(([key, label]) => (
...
function AnggotaSection({ members, isAdmin, busy, onAdd, onEdit, onDelete, onLightbox }) {
  ...
  async function handleEditSave(id) {
    ...
    if (editPhotoFile) {
      try {
        const { default: compress } = { default: compressImage };
        photoUrl = await compressImage(editPhotoFile, 800, 0.85);
      } catch (e) { console.error(e); }
    }
  ...
  for (const m of members) {
    const key = m.position?.split(" ")[0] === "Ketua" || m.position?.toLowerCase().includes("ketua") || m.position?.toLowerCase().includes("wakil") || m.position?.toLowerCase().includes("sekretaris") || m.position?.toLowerCase().includes("bendahara") ? "Pengurus Inti" : (m.division || "Anggota");
  ...
  return (
    <section>
      ...
      {isAdmin && (
        <div style={styles.formCard}>
          ...
          <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 12 }}>
            ...
            {photoPreview && <img src={photoPreview} alt="preview" style={styles.memberPhotoPreview} />}
          </div>
      ...
      {members.length === 0 ? (
        <div style={styles.emptyState}>{isAdmin ? "Belum ada anggota. Tambahkan di atas." : "Belum ada data anggota."}</div>
      ) : (
...
const styles = {
  ...
  main: { maxWidth: 1100, margin: "0 auto", padding: "0 20px 60px", flex: 1, width: "100%" },
  ...
  statRow: { display: "grid", gridTemplateColumns: "repeat(2, 1fr)", background: C.white, borderRadius: "0 0 16px 16px", boxShadow: "0 4px 20px rgba(0,0,0,0.08)", marginBottom: 40 },
  ...
  memberGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 20 },
  ...
  memberPhotoPreview: { width: 60, height: 80, borderRadius: 6, objectFit: "cover", border: "2px solid #c0392b" },
  photoEditOverlay: { position: "absolute", bottom: 0, left: 0, right: 0, background: "rgba(0,0,0,0.5)", color: C.white, textAlign: "center", fontSize: 11, padding: "4px 0", opacity: 0 },
  ...
  footer: { background: C.navyDark, color: C.white, padding: "32px 40px", marginTop: "auto" },
